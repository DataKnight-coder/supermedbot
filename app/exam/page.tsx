...
export default function ExamPage() {
  ...
  if (isExamFinished) return <div className="min-h-screen bg-brand-light" />;
  ...
  if (isGenerating || (isMounted && questions.length === 0)) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[70vh] animate-in fade-in duration-500">
        <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-brand-medical mb-6"></div>
  ...
  if (error) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[70vh] animate-in fade-in duration-500">
        ...
        <p className="text-brand-dark text-xl font-bold">{error}</p>
        <button onClick={() => fetchNextQuestion()} className="mt-6 rounded-xl bg-brand-medical px-8 py-3 text-sm font-bold text-white hover:bg-brand-action transition-colors">
          Retry Sequence
        </button>
      </div>
    );
  ...
  if (!currentQuestion) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[70vh] animate-in fade-in duration-500">
        <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-brand-medical mb-6"></div>
        <p className="text-brand-dark text-lg font-bold">Loading question...</p>
      </div>
    );
  }
  ...
  return (
    <div className="relative max-w-7xl mx-auto px-4 py-8 md:py-12">
      ...
      {toastVisible && (
        <div className="fixed bottom-12 right-12 z-50 bg-brand-medical text-white px-8 py-5 rounded-2xl shadow-2xl animate-in fade-in slide-in-from-bottom-8 duration-500 flex gap-4 items-center">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6 mb-10 border-b border-slate-200 pb-6">
        <div>
            <span className="text-xs font-black text-brand-dark/30 uppercase tracking-widest mb-1 block">{examConfig?.category || 'Mock Exam'}</span>
            <h2 className="font-heading text-3xl font-black text-brand-dark tracking-tight">
              Question {currentQuestionIndex + 1} <span className="text-brand-dark/20">/ {totalQuestions}</span>
            </h2>
        </div>
        ...
        <div className="flex items-center gap-4">
            <button 
              ...
              className={`flex items-center gap-2 px-5 py-2.5 rounded-full border-2 font-bold text-sm transition-all ${
                 isFlagged 
                   ? 'border-brand-medical bg-brand-medical text-white shadow-inner' 
                   : 'border-slate-200 bg-white text-brand-dark/40 hover:border-slate-300 hover:text-brand-dark'
              }`}
            >
              <Flag className={`w-4 h-4 ${isFlagged ? "fill-white" : ""}`} />
              Review
            </button>
            <button onClick={finishExamBlock} className="px-5 py-2.5 rounded-full font-bold text-sm bg-white text-brand-dark/30 border-2 border-slate-200 hover:text-brand-dark hover:border-slate-300 transition-colors">
                End Block
            </button>
            <div className="flex items-center gap-2 bg-brand-medical text-white font-mono text-2xl font-bold px-6 py-2.5 rounded-full shadow-lg ml-2">
               {m}:{s}
            </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
         ...
         <div className="lg:col-span-3">
            <div className="bg-white rounded-3xl p-8 sm:p-12 mb-8 shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-slate-100">
               <p className="text-xl text-brand-dark leading-relaxed font-medium mb-12">
               ...
               <div className="space-y-3">
                  {currentQuestion.options.map((opt) => {
                     ...
                     if (isLocked && currentResult) {
                       ...
                       btnClass += isSelected ? "border-brand-medical bg-brand-action/5 shadow-sm" : "border-slate-200 bg-white hover:border-slate-400";
                       badgeClass += isSelected ? "bg-brand-medical border-brand-medical text-white" : "border-slate-300 bg-slate-50 text-brand-dark/40 group-hover:border-slate-400";
                     }
                     ...
                     return (
                       <button key={opt.key} onClick={() => setAnswer(currentQuestionIndex, opt.key)} className={btnClass} disabled={isLocked}>
                         ...
                         <span className="text-lg font-semibold text-brand-dark">{opt.text}</span>
                       </button>
                     )
                  })}
               </div>
               ...
               {isTutorMode && !isLocked && (
                  <div className="flex justify-end mt-8">
                     <button
                       ...
                       className="rounded-full flex items-center gap-2 bg-brand-medical px-8 py-3.5 text-base font-bold text-white shadow-xl shadow-brand-medical/20 hover:bg-brand-action transition-all active:scale-95 disabled:opacity-50"
                     >
            ...
            <div className="flex justify-between items-center px-4">
               <button 
                  ...
                  className="text-brand-dark/30 font-bold text-base hover:text-brand-dark disabled:opacity-30 transition-colors flex items-center gap-2"
               >
               ...
               {(!isTutorMode || isLocked) && (
                  <button
                     ...
                     className="rounded-full flex items-center gap-2 bg-brand-light text-brand-dark border border-slate-200 px-8 py-3.5 text-base font-black hover:bg-slate-200 transition-all active:scale-95"
                  >
         ...
         <div className="lg:col-span-1 hidden lg:block">
            <div className="bg-white rounded-3xl shadow-sm border border-slate-100 p-8 sticky top-6">
               <h3 className="text-xs font-black text-brand-dark/30 uppercase tracking-widest mb-6 border-b border-slate-50 pb-4">Skip to Question</h3>
                    ...
                    return (
                      <button 
                        ...
                        className={`relative w-full aspect-square rounded-xl border-2 flex items-center justify-center font-bold text-xs transition-all duration-300 ${
                          isCurrent 
                            ? 'border-brand-medical text-brand-medical bg-brand-action/5 ring-4 ring-brand-medical/10 scale-110 z-10' 
                            : hasAnswered 
                              ? 'bg-brand-medical border-brand-medical text-white hover:bg-brand-action hover:border-brand-action hover:scale-105' 
                              : 'bg-white border-slate-200 text-brand-dark/30 hover:border-slate-300 hover:bg-brand-light hover:text-brand-dark/60'
                        }`}
                      >
                        {i + 1}
                        {hasFlagged && (
                          <div className="absolute -top-1.5 -right-1.5 bg-white rounded-full p-0.5 shadow-sm border border-slate-100 z-20">
                             <Flag className="w-3 h-3 fill-brand-medical text-brand-medical" />
                          </div>
                        )}
                      </button>
                    )
